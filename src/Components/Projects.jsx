...
const project = [
    {
      ...
      description: "Web page to find information about the countries where you can filter, search and add information to them.",
      link: "https://countries-app-two-delta.vercel.app/"
    },
    {
      name: "Talkap",
      image: talkap,
      techs: ["React", "Redux", "Node.js", "Sequelize", "Chakra UI", "PostgreSQL", "Socket io", "auth0"],
      description: " Web page of an instant messaging application, allows you to add friends, block chats, rate profiles, and filter by name.",
...
export const Projects = () => {
  ...
  useEffect(() => {
    ...
    return (
        <div id="projects" className="grid gap-6 justify-items-center mt-28 pt-10 pb-10">

         <h2 data-aos="fade-down" data-aos-duration="2000" className="mb-10" style={{textAlign: "left", fontFamily: "Arial", fontSize: "70px", color: "#D946EF", textShadow: "0px 0px 5px #ffffff" }}>Projects</h2>
    ...
    {project.map(p => {
            return(
                ...
                <div data-aos="fade-right" data-aos-duration="2000" key={p.name} className="max-w-xs rounded-lg shadow hover:scale-110 lg:max-w-sm" style={{backgroundColor: 'rgba(255, 253, 253, 0.218)'}}>

                    <a href={p.link} target="_blank" rel='noreferrer'>
                        <img className="rounded-t-lg max-w-xs lg:max-w-sm" src={p.image} alt={p.name}/>
                    </a>
                    ...
                    <div className="p-5">
                        ...
                        <div className="flex gap-2 my-5 flex-wrap">
                                {p.techs.map(t =>
                                    <button key={t} className="bg-great-blue-400 p-1 rounded-lg text-fuchsia-200 text-sm">{t}</button>
                                )}
                            </div>
